...
public class final1 {
	...
	public String findtitle(int i) throws IOException
	{
        ...
        for(int j=0;j<hits.totalHits;j++){
			...
			try {
				if(j%2==0)
					rs.append("<tr class=\"odd\"><td>");
				else
					rs.append("<tr><td>");
				rs.append("<a href=\""+"http://chinese.pku.edu.cn"+doc.get("URL")+"\">");
				fragment = highlighter.getBestFragment(stream, title);
				rs.append(fragment+" </a> </td></tr>");//	System.out.println(fragment);
			} catch (InvalidTokenOffsetsException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
